capFuncs = [

"perf_allow_cpu",
"perf_allow_cpu",
"p4_hw_config",
"p4_hw_config",
"ksys_ioperm",
"__se_sys_iopl",
"subcaches_store",
"store_cache_disable",
"mtrr_open",
"msr_open",
"pfn_modify_allowed",
"mm_access",
"mm_access",
"copy_process",
"copy_process",
"proc_taint",
"r_show",
"proc_do_static_key",
"ns_capable_setid",
"ns_capable_noaudit",
"ns_capable",
"has_capability_noaudit",
"capable_wrt_inode_uidgid",
"capable",
"ptrace_has_cap",
"ptrace_has_cap",
"check_kill_permission",
"set_one_prio",
"__sys_setregid",
"__sys_setregid",
"__sys_setgid",
"__sys_setreuid",
"__sys_setreuid",
"__sys_setuid",
"__sys_setresuid",
"__sys_setresgid",
"__sys_setfsuid",
"__sys_setfsgid",
"__se_sys_sethostname",
"__se_sys_setdomainname",
"do_prlimit",
"__se_sys_prlimit64",
"__se_sys_prctl",
"__se_sys_prctl",
"prctl_set_mm",
"proc_cap_handler",
"proc_cap_handler",
"pid_table_root_permissions",
"alloc_pid",
"alloc_pid",
"copy_namespaces",
"unshare_nsproxy_namespaces",
"cpu_store",
"type_store",
#"force_store", # THIS BUGGER BREAKS THE BUILD BY ENABLING CONFIG_LIVEPATCH
"mode_store",
"hw_protection_store",
"__se_sys_reboot",
"set_permissions",
"may_setgroups",
"__se_sys_setgroups",
"sysctl_schedstats",
"__sched_setscheduler",
"can_nice",
"__se_sys_nice",
"sched_setaffinity",
"snapshot_ioctl",
"check_syslog_permissions",
"proc_dointvec_minmax_sysadmin",
"__se_sys_delete_module",
"__se_sys_init_module",
"__se_sys_finit_module",
"do_adjtimex",
"alarm_timer_create",
"alarm_timer_nsleep",
"timens_install",
"timens_install",
"proc_timens_set_offset",
"__se_sys_acct",
"kexec_load_permitted",
"copy_cgroup_ns",
"cgroupns_install",
"cgroupns_install",
"cgroup1_parse_param",
"cgroup1_get_tree",
"cgroup_release_agent_write",
"remote_cpus_update",
"remote_partition_enable",
"utsns_install",
"utsns_install",
"pid_mfd_noexec_dointvec_minmax",
"pidns_install",
"pidns_install",
"audit_multicast_bind",
"audit_set_loginuid",
"seccomp_actions_logged_handler",
"do_seccomp",
"sysctl_delayacct"
"perf_trace_event_init", # ANOTHER BUGGER NOW THAT WE GOT FILE TRACKING ENABLED, CONFIG_FUNCTION_TRACER breaks the build
"find_get_context",
"find_get_context",
"perf_allow_kernel",
"perf_allow_kernel",
"perf_uprobe_event_init",
"perf_kprobe_event_init",
"perf_kprobe_event_init",
"__se_sys_perf_event_open",
"__se_sys_perf_event_open",
"perf_check_permission",
"perf_check_permission",
"perf_check_permission",
"perf_mmap_calc_limits",
"hw_breakpoint_parse",
"shmem_parse_one",
"account_locked_vm",
"can_do_mlock",
"do_mlock",
"do_mlock",
"do_mlock",
"__se_sys_mlockall",
"__se_sys_mlockall",
"user_shm_lock",
"mlock_future_ok",
"__se_sys_process_madvise",
"__se_sys_swapoff",
"__se_sys_swapon",
"__se_sys_mbind",
"__se_sys_migrate_pages",
"__se_sys_migrate_pages",
"__se_sys_move_pages",
"__se_sys_chroot",
"__x64_sys_vhangup",
"alloc_empty_file",
"mount_capable",
"mount_capable",
"pipe_is_unprivileged_user",
"pipe_is_unprivileged_user",
"alloc_pipe_info",
"pipe_fcntl",
"__check_sticky",
"generic_permission",
"generic_permission",
"generic_permission",
"generic_permission",
"path_init",
"vfs_mknod",
"do_vfs_ioctl",
"do_vfs_ioctl",
"file_ioctl",
"mode_strip_sgid",
"inode_owner_or_capable",
"in_group_or_capable",
"setattr_should_drop_suidgid",
"setattr_prepare",
"setattr_prepare",
"setattr_prepare",
"setattr_prepare",
"mntns_install",
"mntns_install",
"mntns_install",
"clone_private_mount",
"may_mount",
"path_umount",
"path_umount",
"path_umount",
"get_sequential_mnt_ns",
"vfs_open_tree",
"path_mount",
"path_mount",
"do_change_type",
"__se_sys_fsmount",
"__se_sys_move_mount",
"vfs_move_mount",
"vfs_move_mount",
"__se_sys_pivot_root",
"wants_mount_setattr",
"wants_mount_setattr",
"do_mount_setattr",
"__se_sys_statmount",
"__se_sys_statmount",
"__se_sys_listmount",
"__se_sys_listmount",
"xattr_permission",
"simple_xattr_list",
"nsfs_fh_to_dentry",
"vfs_fsconfig_locked",
"init_chroot",
"vfs_dedupe_file_range_one",
"vfs_fileattr_set",
"ep_eventpoll_ioctl",
"do_epoll_ctl",
"__se_sys_timerfd_create",
"do_timerfd_settime",
"vfs_setlease",
"do_handle_open",
"do_handle_open",
"do_handle_open",
"do_handle_open",
"ignore_hardlimit",
"do_quotactl",
"pagemap_read",
"proc_parse_param",
"proc_pid_stack",
"timerslack_ns_write",
"timerslack_ns_show",
"__set_oom_adj",
"__set_oom_adj",
"proc_map_files_get_link",
"proc_map_files_get_link",
"open_kcore",
"ext4_has_free_clusters",
"ext4_fileattr_set",
"ext4_ioctl",
"ext4_ioctl",
"ext4_ioctl",
"ext4_ioctl",
"ext4_ioctl",
"ext4_ioctl",
"swap_inode_boot_loader",
"ext4_resize_begin",
"ext4_attr_store",
"ext4_xattr_trusted_list",
"hugetlb_file_setup",
"fat_generic_ioctl",
"fat_generic_ioctl",
"autofs_root_ioctl_unlocked",
"autofs_dev_ioctl",
"ipcperms",
"ipcctl_obtain_check",
"msgctl_down",
"shmctl_do_lock",
"mqueue_get_inode",
"mqueue_create_attr",
"ipcns_install",
"ipcns_install",
"keyctl_invalidate_key",
"keyctl_keyring_clear",
"keyctl_chown_key",
"keyctl_setperm_key",
"cap_settime",
"cap_ptrace_access_check",
"cap_ptrace_traceme",
"cap_bprm_creds_from_file",
"cap_task_prctl",
"cap_safe_nice",
"cap_convert_nscap",
"cap_convert_nscap",
"cap_inode_setxattr",
"cap_inode_removexattr",
"cap_inode_removexattr",
"mmap_min_addr_handler",
"devcgroup_access_write",
"set_task_ioprio",
"blkpg_do_ioctl",
"blkdev_bszset",
"blkdev_common_ioctl",
"blkdev_common_ioctl",
"blkdev_common_ioctl",
"blkdev_common_ioctl",
"blkdev_common_ioctl",
"blkdev_common_ioctl",
"blkdev_common_ioctl",
"blkdev_common_ioctl",
"blkdev_pr_preempt",
"ioprio_check_cap",
"ioprio_check_cap",
"__se_sys_io_uring_setup",
"io_uring_create",
"proc_bus_pci_read",
"proc_bus_pci_mmap",
"pci_read_config",
"msi_bus_store",
('enable_store', 'drivers/pci/pci-sysfs.c', 323), # Processed at the granular level as there are several functions with this name and it breaks the tool to generically inclide 'em all
"numa_node_store",
"reset_subordinate_store",
"tty_ioctl",
"tty_reopen",
"tiocsti",
"tiocsti",
"tioccons",
"tty_mode_ioctl",
"tty_mode_ioctl",
"tty_ldisc_get",
"tty_jobctrl_ioctl",
"tty_jobctrl_ioctl",
"vt_ioctl",
"vt_ioctl",
"vt_ioctl",
"vt_ioctl",
"vt_ioctl",
"vt_ioctl",
"vt_compat_ioctl",
"set_selection_user",
"vt_do_kdsk_ioctl",
"vt_do_kdsk_ioctl",
"vt_do_kdsk_ioctl",
"vt_do_kdgkb_ioctl",
"tioclinux",
"tioclinux",
"tioclinux",
"tioclinux",
"uart_startup",
"uart_ioctl",
"uart_set_info_user",
"uart_proc_show",
"open_port",
"random_ioctl",
"random_ioctl",
"random_ioctl",
"random_ioctl",
"hpet_ioctl_common",
"nvram_misc_ioctl",
"nvram_misc_ioctl",
"iommu_group_store_type",
"iommu_group_store_type",
"drm_setmaster_ioctl",
"drm_dropmaster_ioctl",
"drm_file_alloc",
"drm_mode_getfb",
"drm_mode_getfb2_ioctl",
"drm_ioctl_kernel",
"i915_getparam_ioctl",
"set_proto_ctx_param",
"validate_priority",
"i915_gem_context_setparam_ioctl",
"i915_gem_context_reset_stats_ioctl",
"i915_gem_execbuffer2_ioctl",
"query_memregion_info",
"query_memregion_info",
"i915_perf_open_ioctl",
"i915_perf_open_ioctl",
"i915_perf_open_ioctl",
"i915_perf_open_ioctl",
"i915_perf_add_config_ioctl",
"i915_perf_add_config_ioctl",
"i915_perf_remove_config_ioctl",
"i915_perf_remove_config_ioctl",
"cn_bind",
"lo_ioctl",
"lo_ioctl",
"lo_ioctl",
"lo_ioctl",
"scsi_cmd_allowed",
"scsi_ioctl_reset",
"scsi_ioctl_reset",
"sdev_store_eh_timeout",
"max_medium_access_timeouts_store",
"max_write_same_blocks_store",
"zeroing_mode_store",
"provisioning_mode_store",
"protection_type_store",
"manage_shutdown_store",
"manage_runtime_start_stop_store",
"manage_system_start_stop_store",
"allow_restart_store",
"sd_ioctl",
"sr_block_ioctl",
"sg_proc_write_dressz",
"sg_proc_write_dressz",
"sg_proc_write_adio",
"sg_proc_write_adio",
"ata_sas_scsi_ioctl",
"ata_sas_scsi_ioctl",
"ata_sas_scsi_ioctl",
"ata_sas_scsi_ioctl",
"cdrom_ioctl",
"cdrom_ioctl",
"cdrom_ioctl",
"serport_ldisc_open",
"rtc_dev_ioctl",
"rtc_dev_ioctl",
"rtc_dev_ioctl",
"pps_cdev_ioctl",
"pps_cdev_ioctl",
"ptp_ioctl",
"pps_enable_store",
"thermal_genl_cmd_threshold_add",
"thermal_genl_cmd_threshold_delete",
"thermal_genl_cmd_threshold_flush",
"rdev_attr_store",
"md_attr_store",
"md_ioctl",
"dm_blk_ioctl",
"dm_ctl_ioctl",
"store_state_disable",
"hda_hwdep_open",
"sock_ioctl",
"sockopt_ns_capable",
"sockopt_capable",
"sk_setsockopt",
"sk_setsockopt",
"sk_setsockopt",
"sk_setsockopt",
"sk_setsockopt",
"sk_setsockopt",
"sk_setsockopt",
"sk_setsockopt",
"sock_bindtoindex_locked",
"sk_set_prio_allowed",
"sk_set_prio_allowed",
"__sock_cmsg_send",
"__sock_cmsg_send",
"sk_ns_capable",
"sk_ns_capable",
"skb_may_tx_timestamp",
"mm_account_pinned_pages",
"__scm_send",
"__scm_send",
"__scm_send",
"netns_install",
"netns_install",
"rtnetlink_bind",
"bpf_sk_base_func_proto",
"bpf_sk_base_func_proto",
"cg_skb_is_valid_access",
"cg_skb_is_valid_access",
"cg_skb_is_valid_access",
"cg_skb_is_valid_access",
"sock_diag_destroy",
"dev_load",
"dev_load",
"dev_ioctl",
"dev_ioctl",
"dev_ioctl",
"net_current_may_mount",
"netdev_lock_store",
"netdev_store",
"napi_defer_hard_irqs_store",
"gro_flush_timeout_store",
"tx_queue_len_store",
"ifalias_store",
"store_rps_dev_flow_table_cnt",
"store_rps_map",
"tx_maxrate_store",
"xps_rxqs_store",
"xps_cpus_store",
"qdisc_set_default",
"netlink_broadcast_filtered",
"__netlink_ns_capable",
"__netlink_ns_capable",
"netlink_bind",
"netlink_connect",
"netlink_setsockopt",
"netlink_setsockopt",
"netlink_allowed",
"genl_bind",
"genl_bind",
"dev_ethtool",
"__ip_options_compile",
"__ip_options_compile",
"__ip_options_compile",
"tcp_ca_find_autoload",
"tcp_set_ulp",
"arp_ioctl",
"devinet_sysctl_forward",
"devinet_ioctl",
"devinet_ioctl",
"__inet_bind",
"inet_create",
"ip_rt_ioctl",
"ip_tunnel_ctl",
"ip_tunnel_ctl",
"ip_mroute_setsockopt",
"unix_ioctl",
"unix_scm_to_skb",
"unix_scm_to_skb",
"__inet6_bind",
"inet6_create",
"ipv6_sock_ac_join",
"addrconf_add_ifaddr",
"addrconf_del_ifaddr",
"ipv6_route_ioctl",
"ip6_datagram_send_ctl",
"ip6_datagram_send_ctl",
"ip6_datagram_send_ctl",
"ipv6_flowlabel_opt",
"fl6_renew",
"fl6_renew",
"mem_check",
"ipip6_tunnel_siocdevprivate",
"ipip6_tunnel_siocdevprivate",
"ipip6_tunnel_ctl",
"__ipip6_tunnel_ioctl_validate",
"packet_create",
"packet_sendmsg",
"packet_sendmsg",
"packet_sendmsg_spkt",
"soft_store",
"state_store.88526",
"net_ctl_permissions",
"security_capable",
"has_ns_capability",
"has_ns_capability_noaudit",
"ns_capable_common",
"ns_capable",
"ns_capable_noaudit",
"has_capability_noaudit",
"ns_capable_setid",
"capable",
"file_ns_capable",
"capable_wrt_inode_uidgid",
"acct",
]

capabilities = [
    "CAP_CHOWN",             # 0
    "CAP_DAC_OVERRIDE",      # 1
    "CAP_DAC_READ_SEARCH",   # 2
    "CAP_FOWNER",            # 3
    "CAP_FSETID",            # 4
    "CAP_KILL",              # 5
    "CAP_SETGID",            # 6
    "CAP_SETUID",            # 7
    "CAP_SETPCAP",           # 8
    "CAP_LINUX_IMMUTABLE",   # 9
    "CAP_NET_BIND_SERVICE",  # 10
    "CAP_NET_BROADCAST",     # 11
    "CAP_NET_ADMIN",         # 12
    "CAP_NET_RAW",           # 13
    "CAP_IPC_LOCK",          # 14
    "CAP_IPC_OWNER",         # 15
    "CAP_SYS_MODULE",        # 16
    "CAP_SYS_RAWIO",         # 17
    "CAP_SYS_CHROOT",        # 18
    "CAP_SYS_PTRACE",        # 19
    "CAP_SYS_PACCT",         # 20
    "CAP_SYS_ADMIN",         # 21
    "CAP_SYS_BOOT",          # 22
    "CAP_SYS_NICE",          # 23
    "CAP_SYS_RESOURCE",      # 24
    "CAP_SYS_TIME",          # 25
    "CAP_SYS_TTY_CONFIG",    # 26
    "CAP_MKNOD",             # 27
    "CAP_LEASE",             # 28
    "CAP_AUDIT_WRITE",       # 29
    "CAP_AUDIT_CONTROL",     # 30
    "CAP_SETFCAP",           # 31
    "CAP_MAC_OVERRIDE",      # 32
    "CAP_MAC_ADMIN",         # 33
    "CAP_SYSLOG",            # 34
    "CAP_WAKE_ALARM",        # 35
    "CAP_BLOCK_SUSPEND",     # 36
    "CAP_AUDIT_READ",        # 37
    "CAP_PERFMON",           # 38
    "CAP_BPF",               # 39
    "CAP_CHECKPOINT_RESTORE" # 40
]
