...
def create_directories(
    kernel, resultdir, new_kern_dir, kern_dir, tar_dir, tarf, ds_recovery, s_config
):
...
def do_compile(
    ...
    if not ds_recovery:
        ...
        if kernel < "linux-2.6.23":
            try:
                subprocess.check_output(
                    "make ARCH={} CROSS_COMPILE={} {} scripts".format(
                        arch, cross, EXTRAVER
                    ),
                    shell=True,
                )
            ...
            try:
                subprocess.check_output(
                    "make ARCH={} CROSS_COMPILE={} {} prepare scripts".format(
                        arch, cross, EXTRAVER
                    ),
                    shell=True,
                )
        ...
        try:
            if kernel < "linux-3.0.0":
                subprocess.check_output(
                    "make ARCH={} CROSS_COMPILE={} {} SUBDIRS=scripts/mod".format(
                        arch, cross, EXTRAVER
                    ),
                    shell=True,
                )
                ...
                subprocess.check_output(
                    "make ARCH={} CROSS_COMPILE={} {} M=scripts/mod".format(
                        arch, cross, EXTRAVER
                    ),
                    shell=True,
                )
        ...
        try:
            ...
            print(
                "There is an error with the {0}st compilation of {1}".format(
                    time, kernel
                )
            )
            print(e)
    ...
    ### Ds recovery single module Compilation
        ...
        try:
            if kernel < "linux-3.0.0":
                subprocess.check_output(
                    "make ARCH={} CROSS_COMPILE={} {} -C {} SUBDIRS={} modules".format(
                        arch, cross, EXTRAVER, image_dir, single_module_dir
                    ),
                    shell=True,
                )
            else:
                subprocess.check_output(
                    "make ARCH={} CROSS_COMPILE={} {} -C {} M={} modules".format(
                        arch, cross, EXTRAVER, image_dir, single_module_dir
                    ),
                    shell=True,
                )
...
def find_and_cscope(image_dir, arch):
    ...
    find_cmd = 'find . -path "./arch/*" ! -path "./arch/{}*" -prune -o -path "./Documentation*" -prune -o -name "Makefile" -print >./cscope.files'.format(
        arch
    )
    ...
    if not ds_recovery:
        ...
        if arch == "mips":
            ...
            if "ARMv6" in ver_magicz:
                defconfig = cu.get_vendor(
                    image, arch, ds_recovery, new_kern_dir, "armv6"
                )
            elif "ARMv7" in ver_magicz:
                defconfig = cu.get_vendor(
                    image, arch, ds_recovery, new_kern_dir, "armv7"
                )
            else:
                defconfig = cu.get_vendor(
                    image, arch, ds_recovery, new_kern_dir, "armv5"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--ds_opt_fl", help="Options for fixing DS alignment", default=None
    )
    parser.add_argument(
        "-l",
        "--ds_opt_list",
        nargs="*",
        help="Option list for fixing DS alignment. Precedence is given to option --ds_opt_fl",
        default=[],
    )
    parser.add_argument(
        "-d",
        "--ds_recovery",
        type=int,
        help="This is used for DS recovery...The FS kernel must be compiled previously (Should have values 0/1)",
        default=0,
    )
    parser.add_argument(
        "-m",
        "--s_mod_dir",
        help="The kernel directory containing the Makefile for the target module...It must be used with ds_recovery",
        default="",
    )
    parser.add_argument(
        "-s",
        "--s_config",
        help="Save the .config file...Used mostly in DS recovery",
        default="yes",
    )
    parser.add_argument(
        "-o",
        "--override_vermagic",
        help="Specify this option to modify the vermagic during KCRE. Used by DSLC",
        action="store_true",
    )
    parser.add_argument(
        "-w",
        "--openwrt",
        help="Specify this option to enable the MIPS OpenWRT patch",
        action="store_true",
    )
    parser.add_argument(
        "-e",
        "--firmadyne",
        help="Include the DSLC fixes for the Firmadyne experiments",
        action="store_true",
    )
