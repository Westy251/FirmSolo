...
# ════════════════════════════════════════════════════════════════════════════
# Hook code templates  (unchanged from original)
# ════════════════════════════════════════════════════════════════════════════
...
__setup_irq = """
        ...
        if (old_temp){
            for (i =irq; i<128; i++){
                desc = irq_to_desc(i);
                old_temp = desc->action;
                if (!old_temp){ irq = i; break; }
            }
        }
        if (desc->chip == &no_irq_chip){ desc->chip = &dummy_irq_chip; }
"""
...
__setup_irq_new = """
        ...
        if (desc->irq_data.chip == &no_irq_chip){ desc->irq_data.chip = &dummy_irq_chip; }
"""
...
setup_irq = """
        ...
        if (desc_temp->chip == &no_irq_chip){ desc_temp->chip = &dummy_irq_chip; }
"""
...
hooks_dict = {
    ...
    "sys_reboot3": [
        sys_reboot_old,
        fl_sys_reboot,
        "only_between linux-2.6.28 linux-2.6.29",
        "sys_reboot",
        [],
    ],
    "sys_reboot4": [
...
def create_cscope_db():
    try:
        subprocess.call("cscope -q -b", shell=True)
    except Exception:
        print(traceback.format_exc())
...
def modify_file(kfile, wts, tmplt, touched_files):
    ...
    # ── Find the function definition via cscope ───────────────────────────────
    res = run_cscope(wts, level)

    # CHANGED: initialise matched_tokens before use (was undefined in original)
    matched_tokens = None
    if res:
        for rs in res.split("\n"):
            tokens = rs.split()
            # Guard: need at least file, symbol, line_num
            if len(tokens) < 3:
                continue
    ...
    # CHANGED: fixed inverted if/else that made start_line unreachable
    if matched_tokens:
        start_line = (
            int(matched_tokens[2])
            if not header_included
            else int(matched_tokens[2]) + 1
        )
    else:
        print(f"Could not find valid match for {wts}")
        return

    # ── Insert the template after the opening brace ───────────────────────────
    for i, line in enumerate(lines[int(start_line) :]):
        if not end and "{" in line:
            index = i + int(start_line) + 1
            for ln in tmplt:
                lines.insert(index, ln)
                index += 1
            break
        elif end and ("return nr" in line or "return pid" in line):
            if "pid" in line:
                tmplt[2] = tmplt[2].replace("nr)", "pid)")
...
def fix_main():
    # CHANGED: fixed typo 'ingore' → 'ignore'
    with open("init/main.c", "r", errors="ignore") as f:
        lines = f.readlines()
    ...
    for i, line in enumerate(lines):
        if (
            ...
            lines[i] = (
                'char * envp_init[MAX_INIT_ENVS+3] = { "HOME=/", "TERM=linux",'
                ' "LD_PRELOAD=/firmadyne/libnvram.so", NULL,  };\n'
            )
            index = i - 2
            for ln in template:
                lines.insert(index, ln)
...
def apply_fdyne_hooks(kern_source, kernel):
    ...
    # CHANGED: wrapped in try/except so a missing init/main.c doesn't abort
    try:
        fix_main()
    except FileNotFoundError as e:
        print(f"firmadyne_fix: could not fix init/main.c: {e}")
    ...
    for key in sorted(list(hooks_dict.keys()), reverse=True):
        ...
        if len(data) > 4:
            ...
            if tokens[0] == "only_between":
                if (kernel < tokens[1] or kernel >= tokens[2]) and kernel not in data[
                    4
                ]:
                    continue
                else:
            ...
            if tokens[0] == "below":
                ...
                min_k, max_k = tokens[1], tokens[2]
                if kernel >= min_k and kernel < max_k or kernel in data[4]:
                    what_to_search = data[3]
                else:
                    continue

            # CHANGED: guard against missing file before patching
            if not os.path.exists(fl_to_modify):
                print(f"firmadyne_fix: {fl_to_modify} not found – skipping {key}")
                continue
        ...
        # CHANGED: guard against missing file
        if not os.path.exists(fl_to_modify):
            print(f"firmadyne_fix: {fl_to_modify} not found – skipping {key}")
            continue
        modify_file(fl_to_modify, what_to_search, template, touched_files)
