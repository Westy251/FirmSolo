...
conf_match = [
    ...
    "CONFIG_ARCH_VERSATILE CONFIG_CPU_ARM926T",
    "CONFIG_ARCH_REALVIEW CONFIG_REALVIEW_EB_ARM11MP CONFIG_MACH_REALVIEW_PB11MP CONFIG_REALVIEW_EB_ARM11MP_REVB CONFIG_CPU_V6",
    "CONFIG ARCH_REALVIEW CONFIG_MACH_REALVIEW_EB CONFIG_REALVIEW_EB_A9MP CONFIG_MACH_REALVIEW_PBA8 CONFIG_MACH_REALVIEW_PBX CONFIG_CPU_V7",
        ...
        for i, term in enumerate(self.split_expr(expr, split_op)):
            if isinstance(target, tuple):
                ...
                if target.visibility > 0:
                    ...
                    if not isinstance(term, tuple) and term.tri_value == 0:
                        print(
                            "However target {0} is not enabled...Setting the target".format(
                                target.name
                            )
                        )
            ...
            if isinstance(term, tuple):
                ...
                if expr_value(term) > 0 and term[0] != NOT:
                    print(
                        "The subdep {0} is already satisfied continuing...".format(
                            expr_str(term)
                        )
                    )
                    continue
                ...
                if term[0] == NOT and len(term) == 2:
                    if isinstance(term[1], tuple):
                        print(
                            "Found complex NOT expr {0} Aborting...".format(
                                expr_str(term)
                            )
                        )
                        continue
                        ...
                        print(
                            "Found simple NOT expr {0} Not Aborting...".format(
                                expr_str(term[1])
                            )
                        )
            ...
            # The dependency is just a symbol we must find its type
                ...
                if isinstance(term, str):
                    print(
                        "Simple Symbol/String dep {0} not breaking further down".format(
                            term
                        )
                    )
                    ...
                    if term.name == None:
                        return
                    print(
                        "Simple Symbol/String dep {0} with value {1} not breaking further down".format(
                            term.name, term.tri_value
                        )
                    )
    ...
    def set_option(self, conf_opt, value, overwrite=False):
        ...
        if isinstance(sym, (Symbol, Choice)):
            ...
            if sym.type == BOOL:
                if value == 1:
                    print(
                        "Type of symbol",
                        sym.name,
                        "is bool thus the value cannot be set to 1",
                    )
            ...
            if value in sym.assignable:
                print(
                    "The value {0} can be set directly for {1}...Setting the value".format(
                        value, option
                    )
                )
                self.set_option_value(sym.name, value, overwrite)
                print(
                    "Config string to write to the .config file for option {0} is {1} and new visibility {2}".format(
            ...
            if sym.visibility == 0:
                print(
                    "Still not visible... trying to set symbol",
                    sym.name,
                    "by using its reverse dependencies (selected by)",
                )
            ...
            print(
                "Config string to write to the .config file for option {0} is {1} and new visibility {2}, assignable {3}".format(
                ...
                # then try to set it else abort
                    ...
                    if (
                        ...
                        if term.tri_value != value:
                            print(
                                "Still not equal to value",
                                value,
                                " ... trying to set symbol",
                                term.name,
                                "by using its reverse dependencies (selected by) to value",
                                value,
                            )
                        ...
                        if value in term.assignable:
                            ...
                            print(
                                "Setting to the value of direct dep",
                                expr_str(term.direct_dep),
                            )
...
def find_custom_module_options(modulez):
    ...
    print(
        "Finding the config options pertaining to the counterpart upstream modules of the custom modules"
    )
